...
#         diffs.append(f"Text mismatch at {current_path}: '{elem1.text.strip() if elem1.text else ''}' != '{elem2.text.strip() if elem2.text else ''}'")

#     if elem1.attrib != elem2.attrib:
#         for key in set(elem1.attrib.keys()).union(elem2.attrib.keys()):
#             val1 = elem1.attrib.get(key)
#             val2 = elem2.attrib.get(key)
#             if val1 != val2:
#                 diffs.append(f"Attribute '{key}' mismatch at {current_path}: '{val1}' != '{val2}'")
...
#     # Write output to file in script's directory
...
# file1 = input("\nEnter the path to the first XML file: ").strip()
# file2 = input("Enter the path to the second XML file: ").strip()
# print()

# # Output file name
# output_file = "xml_diff_result.txt"

# # Always write results, even if input files are missing
